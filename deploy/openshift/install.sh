...
deploy_wva_controller() {
    ...
    kubectl get secret thanos-querier-tls -n openshift-monitoring -o jsonpath='{.data.tls\.crt}' > ${PROM_CA_CERT_PATH}
    PROM_CA_CERT=$(< ${PROM_CA_CERT_PATH})
    ...
    -n $WVA_NS \
    --set wva.prometheus.caCert=${PROM_CA_CERT} \
